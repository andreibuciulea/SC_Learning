...
Ps = 0.95:-0.05:0.55;
nps = numel(Ps);
err = zeros(nG,5,nM,nps);
tic
parfor g = 1:nG
    err_g = zeros(5,nM,nps);
    ...
    for r = 1:nps
        ...
        for m = 1:nM 
            ...
            out = estimate_SC(Xm0,Xm1,E,T,model,input,B2,w1);
            ...
            %err_g(2,m,r) = norm(B2-B2_hat,'fro')^2/norm(B2,'fro')^2;
...
mrkrs = {'-s',':^','--x'};
metrics = {'B1','B2','L0','LB2','X1'};
figure()
i = 1;
for k = [3,4]
    for m = 1:nM
        lgd{i} = [metrics{k} ' ' Models{m}];
        semilogy(Ps*100,squeeze(merr(k,m,:)),mrkrs{m},LineWidth=2)
        hold on
        i = i+1;
    end
end
legend(lgd)
xlabel('Percentage of available edge signals')
